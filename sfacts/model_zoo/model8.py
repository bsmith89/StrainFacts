...
def model(
    ...
    with pyro.plate("position", g, dim=-1):
        with pyro.plate("strain", s, dim=-2):
            _gamma_delta = pyro.sample(
                "_gamma_delta",
                dist.Dirichlet(
                    concentration=gamma_hyper.repeat(3),
                ),
            )
    delta = pyro.deterministic("delta", 1 - _gamma_delta[:, :, 2])
    gamma = pyro.deterministic("gamma", _gamma_delta[:, :, 1] / (1 - delta))
